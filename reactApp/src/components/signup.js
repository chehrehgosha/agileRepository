...
async handleSignUp() {
    ...
    var userReg = new RegExp("[^A-Za-z0-9_@\.]|@{2,}|\.{5,}");
    var passReg = new RegExp("^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{4,8}$");
    var emailReg = new RegExp("[\w-]+@([\w-]+\.)+[\w-]+");
    if (passReg.test(values[2].value)) {
    if (userReg.test(values[0].value)) {
      if (emailReg.test(values[1].value)) {
        } else {
          alert("Invalid Combination of E-mail");
          return;
        }
      }
      else {
        alert("Invalid Combination of Username");
        return;
      }
    } else {
      alert("Invalid Combination of Password");
      return;
    }
    ...
    return (<div className={"signin-holder"}>
      <form id='signup-form'>
        <h1>Sign Up</h1>
          ...
          <label htmlFor="password">Password (must at least contain an uppercase, lowercase, numeric digit):</label>

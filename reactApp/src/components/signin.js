...
async handleSignIn() {
  ...
  const user = await Parse.User.logIn(values[0].value, values[1].value).then(() => {
    ...
    return (<div className={"signin-holder"}>
      <form id='signin-form'>
        <h1>Sign In</h1>
          ...
          <input type="password" id="password" name="user_password" pattern="[a-z0-9]{6,25}"/>


          <Link onClick={this.handleSignIn} to={"/"}><button id="signupButton" type="submit">
         Sign In
        </button></Link>
      </form>
    </div>)
